...
db_mongo = client['find_job']
vacancies = db_mongo.vacancies
...
while True:
    ...
    for vacancy in vacancy_list:
        ...
        # добавляем данные по элементу в result
        ...
        vacancies.insert_one({'name': vac_name,
                              'min salary': vac_min_salary,
                              'max salary': vac_max_salary,
                              'employer': vac_employer.text if vac_employer else '',
                              'place': vac_place,
    ...
    if next_link:
        # здесь можно было бы понадежнее сделать - вместо инкремента выбирать номера страницы из ссылки
...
while True:
    ...
    for vacancy in vacancy_list:
        ...
        # парсим работодателя
        vac_employer = vacancy.find('span', {
    ...
    if next_link:
        # здесь тоже можно было бы понадежнее сделать - вместо инкремента выбирать номера страницы из ссылки
        params['page'] = str(int(params['page']) + 1)
    else:
        break
...
def search():
    ...
    for item in vacancies.find({'min salary':{'$gt':a}}, {'max salary':{'$gt':a}}):
        pprint(item)
...
# result_db = db.to_json(orient="records")
# print(result_db)
...
# db_mongo = client['find_job']
# vacancies = db_mongo.users
# vacancies.insert_many(db.to_dict())
